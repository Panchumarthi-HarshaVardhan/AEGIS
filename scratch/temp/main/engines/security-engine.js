...
class SecurityEngine {
     ...
     */
    constructor(secretScanner, riskClassifier, phishingDetector) {
     ...
     */
    async evaluate(intent, rawInput) {
        ...
        if (detectedSecrets.length > 0) {
            const criticalSecrets = detectedSecrets.filter((s) => s.pattern === 'critical');
            ...
            const effectiveRisk = Math.max(riskClassification.level, 2);
            return {
                approved: false,
     ...
     */
    async analyzeUrlIfPresent(intent) {
