...
export interface ActionStep {
  /** The action identifier (e.g., 'open_app', 'open_url', 'search_for') */
  action: string
...
/**
 * A complete plan for executing an intent.
 */
export interface ActionPlan {
  /** Ordered list of steps to execute */
  steps: ActionStep[]
  /** Whether steps must be executed in sequence (true) or can be parallelized (false) */
...
export class PlannerEngine {
  /**
   * Verifies that the planned steps are logically correct, contain valid actions,
   * and have non-empty parameters.
   */
  public verify(plan: ActionPlan): VerificationResult {
    const errors: string[] = []

    if (!plan || !plan.steps) {
      return { valid: false, errors: ['Action plan or steps array is undefined'] }
    }

    if (plan.steps.length === 0) {
      return { valid: false, errors: ['Action plan does not contain any steps'] }
    }
    ...
    for (let i = 0; i < plan.steps.length; i++) {
      ...
      if (!step.action) {
        errors.push(`Step ${stepNum} is missing an action identifier.`)
        continue
      }

      if (!validActions.includes(step.action)) {
        errors.push(`Step ${stepNum} has an unrecognized or unsupported action: "${step.action}".`)
      }

      if (!step.params) {
        errors.push(`Step ${stepNum} (${step.action}) is missing the params object.`)
      }

      if (step.risk_level === undefined || step.risk_level < 0 || step.risk_level > 3) {
        errors.push(`Step ${stepNum} (${step.action}) has an invalid risk level: ${step.risk_level}.`)
      }
    }
   ...
   */
  plan(intent: ParsedIntent): ActionPlan {
   ...
   */
  private planOpenApp(intent: ParsedIntent): ActionPlan {
   ...
   */
  private planOpenUrl(intent: ParsedIntent): ActionPlan {
   ...
   */
  private planSearchWeb(intent: ParsedIntent): ActionPlan {
   ...
   */
  private planPlayMusic(intent: ParsedIntent): ActionPlan {
   ...
   */
  private planSearchProduct(intent: ParsedIntent): ActionPlan {
    const query = intent.entities.query ?? ''
   ...
   */
  private planSummarize(intent: ParsedIntent): ActionPlan {
    const query = intent.entities.query ?? ''
    const url = intent.entities.url
   ...
   */
  private planFileOperation(intent: ParsedIntent): ActionPlan {
   ...
   */
  private planUnknown(intent: ParsedIntent): ActionPlan {
    return {
      steps: [
        {
          action: 'noop',
   ...
   */
  private buildSiteSearchUrl(website: string, query: string): string {
   ...
   */
  private getProductSiteUrl(website: string): string {
   ...
   */
  private buildProductSearchUrl(website: string, query: string): string | null {
    const encoded = encodeURIComponent(query)
   ...
   */
  private mapSystemControlAction(action: string): string {
